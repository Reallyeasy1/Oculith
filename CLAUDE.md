...
**Windows caveat:** `npm run check` has known platform artifacts, not bugs. `container-codex-runner.test.ts` always fails (expects POSIX paths; `path.resolve("/tmp/...")` yields `C:\tmp\...`), and under machine load a floating handful of other suites can exceed vitest's 5s default on cold imports or wall-clock perf bounds (`app.test.ts`, `workspace-browse.test.ts`, `glassbox.integration.test.ts`, `metrics.test.ts` have all flaked; each passes rerun in isolation or with `--testTimeout=30000`). Typecheck, the rest of the ~670-test suite, and `build` pass on Windows; treat the suite as authoritative only on Linux/macOS (where CI runs it green).
...
Parallel sessions/agents (rule: `.claude/rules/parallel-work.md`): **assign the issue to yourself before anything else** — `bash scripts/dev/claim-issue.sh N` (assignee + label `in-progress` + claim-token comment; refuses claimed/closed/branch-exists and backs off if it loses the race). The guard hook refuses to create `feat|fix|chore/N-*` unless N is `in-progress` and assigned to you, and refuses a second session on this machine for the same issue. One issue = one branch = one worktree = one session: the session that creates (or first commits to) a branch owns it; other sessions cannot commit/push it (`OCULITH_OWNER_OVERRIDE=1` only for a controller taking over a finished agent branch). PRs only via `/finish-issue` (a second PR for the same head is blocked); merges only via `bash scripts/dev/merge-prs.sh <pr…>` (requires a `## Review — Mergeable` comment, retargets dependents before deleting the head branch; manual `gh pr merge`, `git push --delete` and pushes to `main` are blocked).
