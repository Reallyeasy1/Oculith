...
- **Web view models**: `needsAttention` = error ∪ timeout ∪ cancelled ∪ degraded ∪ `denials > 0` ∪ `toolFailures > 0` (#131); `recoveredFailures` for ok Runs; `liveRuns`; `capabilityCopy` / `capabilityBadgeLabel` (pending while running, `model: no evidence` after; long form in the tooltip), `evidenceBadges` (neutral `no tool calls` on ok Runs; layer-specific `no evidence` warnings only on ended non-ok Runs, #137); `spanStatusLabel` (`interrupted` for incomplete spans of a restart-ended Run); `timelineTicks`, `barGeometry` (open-ended bars for incomplete spans, instant markers for 0 ms).
