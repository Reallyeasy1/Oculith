...
_What has been tested, how, and what has not. Last updated 31 August 2026 (main `9510617`). Automated coverage comes from `npm run check` (unit tests, guard self-test, build) and the E2E lane (`npm run test:e2e`); manual coverage comes from the ten UAT rounds below. This record is updated at the end of every UAT round._
...
| Policy denials (#81) | observer + query tests | — | 7 | real `policy.denied` evidence on Windows local-process (20–25 denials/Run, round 7); still not demonstrable on the judged Docker path (`danger-full-access`) → recorded export, see #92 |
...
| Workspace templates (#68) | workspace tests (wx writes, bad template) | — | 4 (API), 7 (Create-Agent-from-template through the form) | ok |
...
| Auth (bearer token) | app test | [1] 401 without token | 7 (browser flow with `APP_AUTH_TOKEN` set), 10 (401 on missing/wrong token) | ok |
...
- `setsTaskOutcome` writing `taskOutcome` through a UI-driven flow (`task_completion` scoring and a real `post_check` execution were verified in the 29 Aug judged-path rehearsal and round 10's judge-score sweep — see docs/DEMO.md and the round-10 report).
- The ≤ 800 px span-drawer overlay branch (round 6 measured 1920/1366/1024).
- The Jump-to-failing-span flow on a failed Run through manual UAT (E2E [6] covers it; the demo rehearsals exercised it live). The `REGRESSION` comparison branch was driven end to end in the 28 Aug dev rehearsal and both judged rehearsals (docs/DEMO.md) — remaining gap is only a manual UAT round observing it.
...
- The #234 template-mismatch banner (needs a template edited between two EvalRuns).
