...
_What has been tested, how, and what has not. Last updated 28 August 2026 (main `ce0c186`). Automated coverage comes from `npm run check` (unit tests, guard self-test, build) and the E2E lane (`npm run test:e2e`); manual coverage comes from the five UAT rounds below. This record is updated at the end of every UAT round._
...
- Template-hash mismatch 409 and `force` through the API (needs a copy of a template edited after the case was recorded); the dashboard cannot send `force` yet.
