...
| 0.111 / 0.142 | `item.completed` | `agent_message` | `id`, `text` | final output = **last** agent message; not stored as trace content; counted toward `modelCallsObserved` on the turn end (#207) | observed | E6 |
| 0.111 / 0.142 | `item.completed` | `reasoning` | `id`, `text` | **dropped — no chain-of-thought; deliberately not mapped**; only counted, one model call each, toward `modelCallsObserved` on the turn end (#207) | observed | E7 |
