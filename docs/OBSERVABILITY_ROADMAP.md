...
Raw prompts/completions at the default policy, **raw** chain-of-thought text at any policy (under the explicit opt-in `reasoning_summary` tier, reasoning appears only as 240-char redacted summaries — never raw, never by default, #259), headers/env/credentials (redaction denylist + fail-closed), synthetic spans for missing runtime detail, LLM-generated diagnosis text. Every "capture more" proposal above is metadata, a hash, a count, or an already-bounded redacted summary — the invariants are the moat, and the judging story leads with them.
