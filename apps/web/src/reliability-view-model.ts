...
/** Bar heights as 0–100 % of the series max; null stays null (no observation is not zero). */
export function sparklineHeights(values: readonly (number | null)[]): (number | null)[] {
  const observed = values.filter((value): value is number => value !== null);
  const max = observed.length === 0 ? 0 : Math.max(...observed);
