...
const form = { name: "Builder", description: "", instructions: "Help", workspace: "", template: "" };

describe("agentPayload", () => {
  it("omits empty workspace and template so the default Create Agent is not a 400", () => {
    expect(agentPayload(form, { template: true })).toEqual({ name: "Builder", description: "", instructions: "Help" });
  });
  it("sends the chosen workspace and template on create, never the template on save", () => {
    ...
    expect(agentPayload(chosen, { template: false })).toEqual({ name: "Builder", description: "", instructions: "Help", workspace: "shared-repo" });
  });
});
