...
(async () => {
  ...
  eq((await page.locator(".runs-table tbody tr").first().locator("td").nth(1).innerText()).trim(), "managed", "managed workspace UUID is rendered as 'managed'");
  eq(await page.locator(".runs-table tbody tr").first().locator("td").nth(1).getAttribute("title"), agent.id, "managed workspace keeps its UUID in the tooltip");
