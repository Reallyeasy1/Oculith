...
- **Polling already exists** for Run status; hook approval and event fetches into that loop rather than adding timers. The single sanctioned exception is the 5 s `setInterval` in `Overview.tsx` (#70): the All-runs overview has no selected Agent to drive that loop, so nothing else would surface a Run finishing under another Agent; it lives in a `useEffect` and is cleared on unmount.
