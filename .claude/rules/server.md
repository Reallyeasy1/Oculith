...
- **All state changes go through `store.mutate()`**; read with `store.snapshot()`. One `mutate` per logical transaction (e.g. create Run + Lease + `run_started` event together) so partial writes can't happen.
...
- **New gateway logic lives in `apps/server/src/launchguard/`** (evaluator, canonicalise, lease, approval, redact, adapter) as pure functions where possible; `agent-service.ts` orchestrates, `app.ts` exposes.
