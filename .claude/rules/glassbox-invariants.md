...
5. **No chain-of-thought, no raw content.** Capture policy is `metadata_only` by default, `safe_summary` opt-in with bounded, filtered text; `full/raw` is not implemented — do not add it.
