...
4. Map to seams — name the exact files you will touch, using CLAUDE.md's architecture section: `types.ts` (contracts), `app.ts` (routes + zod), `agent-service.ts` (Run lifecycle), `codex-runner.ts` / `container-codex-runner.ts` (Runtime boundary, env allow-lists), `store.ts` (persistence), `apps/web/src/App.tsx` + `types.ts` (UI, mirrored types), `Dockerfile.runtime` (agentctl). Prefer a new `apps/server/src/launchguard/` module for gateway logic.
